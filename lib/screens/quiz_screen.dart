...
// // _SpellingQuizResultScreen이 이 파일 안에 있으므로 아래 import는 주석 처리하거나 삭제합니다.
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            ...
            const SizedBox(height: 30),
            // 핵심 수정 1: TextField를 Opacity로 감싸서 화면에만 보이지 않게 처리
            Opacity(
              opacity: 0,
              child: TextField(
                controller: _textController,
                focusNode: _focusNode,
                autofocus: false, // 충돌 방지를 위해 false로 설정
                enableSuggestions: false,
                autocorrect: false,
                // enabled 속성을 제거하여 키보드 깜빡임 문제 해결
                onSubmitted: (_) {
                  _checkAnswer();
                },
              ),
            ),
            // 핵심 수정 2: 사용자가 입력 영역을 탭하면 키보드를 올리도록 GestureDetector 추가
